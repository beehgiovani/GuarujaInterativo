...
window.Tracking = {
    ...
    logVisit: async function() {
        ...
        try {
            ...
            // 2. Registrar no Supabase usando upsert (evita erro 409/duplicate)
            const { error } = await window.supabaseApp.from('visitor_stats').upsert({
                ip_hash: ip,
                visited_at: new Date().toISOString().split('T')[0]
            }, { onConflict: 'ip_hash,visited_at' });
    ...
    getStats: async function() {
        try {
            ...
            const realOnline = onlineRes.count || 0;
            
            // Prova Social: (Número entre 234 e 254) + usuários reais
            // Usamos um offset fixo baseado no dia para não ficar pulando loucamente
            const dayOfYear = Math.floor((Date.now() - new Date(new Date().getFullYear(), 0, 0)) / (1000 * 60 * 60 * 24));
            const baseOffset = 234 + (dayOfYear % 17); // Varia entre 234 e 250 conforme o dia
            
            // Garantir que não seja redondo (ex: se der 240, vira 241)
            let onlineDisplay = baseOffset + realOnline;
            if (onlineDisplay % 10 === 0) onlineDisplay += 1;

            return {
                uniqueVisitors: (visitsRes.count || 0) + 1200, 
                onlineUsers: onlineDisplay,
                realOnline: realOnline // Guardado para o Painel Master
            };
        } catch (e) {
            console.error("Erro ao buscar stats:", e);
            return { uniqueVisitors: 1200, onlineUsers: 237, realOnline: 0 };
        }
    },
