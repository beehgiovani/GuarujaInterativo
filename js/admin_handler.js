...
window.AdminHandler = {
    ...
    renderMonetization: async function(container) {
        ...
        container.innerHTML = `
            <div style="display: flex; flex-direction: column; gap: 24px;">
                ...
                <div>
                    <div style="font-size: 11px; color: #94a3b8; text-transform: uppercase; font-weight: 700; margin-bottom: 12px; letter-spacing: 1px;">📊 Referência de Tiers Vigente</div>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px;">
                        ${[
                            { id: 'user', nome: 'Gratuito', preco: 'R$ 0', fichas: '0', cor: '#64748b' },
                            { id: 'start', nome: plansConfig?.start?.name || 'Start', preco: `R$ ${plansConfig?.start?.price || 97}/mês`, fichas: `${plansConfig?.start?.credits || 15}/mês`, cor: '#0d9488' },
                            { id: 'pro', nome: plansConfig?.pro?.name || 'Pro', preco: `R$ ${plansConfig?.pro?.price || 179}/mês`, fichas: `${plansConfig?.pro?.credits || 40}/mês`, cor: '#2563eb' },
                            { id: 'elite', nome: plansConfig?.elite?.name || 'Elite', preco: `R$ ${plansConfig?.elite?.price || 349}/mês`, fichas: `${plansConfig?.elite?.credits || 100}/mês`, cor: '#7c3aed' },
                            { id: 'vip', nome: plansConfig?.vip?.name || 'Anual VIP', preco: `R$ ${plansConfig?.vip?.price || 2990}/ano`, fichas: `${plansConfig?.vip?.credits || 120}/mês`, cor: '#1e293b' },
                            ...
                            <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.07); border-radius: 10px; padding: 12px;">
                                <div style="font-size: 8px; color: #64748b; text-transform: uppercase; font-weight: 700;">${p.id}</div>
                                <div style="font-size: 13px; font-weight: 800; color: white; margin: 2px 0;">${p.nome}</div>
                                <div style="font-size: 11px; color: ${p.cor}; font-weight: 600;">${p.preco}</div>
                                <div style="font-size: 9px; color: #475569; margin-top: 2px;">${p.fichas}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
                ...
                <!-- SEÇÃO 6: GESTÃO DE PREÇOS E LIMITES (v2) -->
                <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 20px;">
                    <div style="font-size: 11px; color: #94a3b8; text-transform: uppercase; font-weight: 700; margin-bottom: 20px; letter-spacing: 1px;">💰 Gestão de Preços e Promoções</div>
                    <div id="plans-config-container" class="admin-plans-grid" style="display: flex; flex-direction: column; gap: 14px;">
                        <div class="admin-grid-header" style="display: grid; grid-template-columns: 80px 140px 100px 100px 80px 100px; gap: 15px; align-items: center; padding-bottom: 10px; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 9px; font-weight: 800; color: #64748b; text-transform: uppercase;">
                            <div>ID</div>
                        ...
                        ${['start', 'pro', 'elite', 'vip'].map(role => {
                            const p = plansConfig?.[role] || {};
                            const color = role === 'start' ? '#0d9488' : (role === 'pro' ? '#2563eb' : (role === 'elite' ? '#7c3aed' : '#ffffff'));
                            return `
                                <div class="admin-grid-row" style="display: grid; grid-template-columns: 80px 140px 100px 100px 80px 100px; gap: 15px; align-items: center;">
                                    <div style="font-weight: 800; color: ${color}; font-size: 11px;">${role.toUpperCase()}</div>
                                    <input id="plan-name-${role}" type="text" value="${p.name || ''}" placeholder="Ex: ${role.toUpperCase()}" style="padding: 6px 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; color: white; font-size: 12px;" />
                                    <input id="plan-price-${role}" type="number" value="${p.price || 0}" style="padding: 6px 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; color: white; font-size: 12px;" />
                                    <input id="plan-price-tabela-${role}" type="number" value="${p.price_tabela || 0}" style="padding: 6px 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; color: white; font-size: 12px;" />
                                    <input id="plan-credits-${role}" type="number" value="${p.credits || 0}" style="padding: 6px 10px; background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; color: white; font-size: 12px;" />
                                    <div style="text-align: center;">
                                        <input id="plan-promo-${role}" type="checkbox" ${p.promo_active ? 'checked' : ''} style="cursor: pointer;" />
                                    </div>
                                </div>
                            `;
                        }).join('')}

                        <div style="margin-top: 10px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.05);">
                            <button onclick="window.Admin.savePlansConfig()" style="background: #10b981; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-size: 13px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 8px;">
                                <i class="fas fa-check-double"></i> Salvar Alterações de Preços
                            </button>
                        </div>
                    </div>
                </div>
    ...
    savePlansConfig: async function() {
        ...
        try {
            const config = {
                start: {
                    name: document.getElementById('plan-name-start')?.value || 'START',
                    price: parseFloat(document.getElementById('plan-price-start')?.value) || 97,
                    price_tabela: parseFloat(document.getElementById('plan-price-tabela-start')?.value) || 149,
                    promo_active: document.getElementById('plan-promo-start')?.checked ?? true,
                    promo_price: parseFloat(document.getElementById('plan-price-start')?.value) || 97,
                    credits: parseInt(document.getElementById('plan-credits-start')?.value) || 15
                },
                pro: {
                    name: document.getElementById('plan-name-pro')?.value || 'PRO',
                    price: parseFloat(document.getElementById('plan-price-pro')?.value) || 179,
                    price_tabela: parseFloat(document.getElementById('plan-price-tabela-pro')?.value) || 299,
                    promo_active: document.getElementById('plan-promo-pro')?.checked ?? true,
                    promo_price: parseFloat(document.getElementById('plan-price-pro')?.value) || 179,
                    credits: parseInt(document.getElementById('plan-credits-pro')?.value) || 40
                },
                elite: {
                    name: document.getElementById('plan-name-elite')?.value || 'ELITE',
                    price: parseFloat(document.getElementById('plan-price-elite')?.value) || 349,
                    price_tabela: parseFloat(document.getElementById('plan-price-tabela-elite')?.value) || 599,
                    promo_active: document.getElementById('plan-promo-elite')?.checked ?? true,
                    promo_price: parseFloat(document.getElementById('plan-price-elite')?.value) || 349,
                    credits: parseInt(document.getElementById('plan-credits-elite')?.value) || 100
                },
                vip: {
                    name: document.getElementById('plan-name-vip')?.value || 'ANUAL VIP',
                    price: parseFloat(document.getElementById('plan-price-vip')?.value) || 2990,
                    price_tabela: parseFloat(document.getElementById('plan-price-tabela-vip')?.value) || 4990,
                    promo_active: document.getElementById('plan-promo-vip')?.checked ?? true,
                    promo_price: parseFloat(document.getElementById('plan-price-vip')?.value) || 2990,
                    credits: parseInt(document.getElementById('plan-credits-vip')?.value) || 120,
                    billing: 'annual'
                }
            };
                ...
                <div class="custom-modal-body" style="padding: 24px; display: flex; flex-direction: column; gap: 14px;">
                    ...
                    <div>
                        ...
                        <select id="manual-plan-role" style="width: 100%; margin-top: 6px; padding: 10px; background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; color: white; font-size: 13px;">
                            ...
                            <option value="elite">ELITE — R$ 349/mês</option>
                            <option value="vip">ANUAL VIP — R$ 2.990/ano</option>
    ...
    renderTransactions: async function(container) {
        ...
        try {
            ...
            // Health Metrics (Finance)
            const costPerCredit = 0.32; // Custo Datastone por consulta/lote
            const totalCreditsUnlocked = 0; // Seria ideal buscar das txs aprovadas
            const estimatedCost = revenue > 0 ? (revenue * 0.1) : 0; // Mock de custo operacional p/ demo
            const netProfit = revenue - estimatedCost;
            const breakEven = 1200; // Exemplo de custo fixo mensal (Servidor + APIs)

            container.innerHTML = `
                <!-- Dashboard de Saúde Financeira -->
                <div style="background: rgba(16, 185, 129, 0.05); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: 12px; padding: 20px; margin-bottom: 25px;">
                    <div style="font-size: 11px; color: #10b981; text-transform: uppercase; font-weight: 800; margin-bottom: 15px; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-chart-line"></i> Saúde Financeira do Projeto
                    </div>
                    <div class="admin-health-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 15px;">
                        <div>
                            <div style="font-size: 10px; color: #94a3b8; text-transform: uppercase;">Margem (Líquido)</div>
                            <div style="font-size: 20px; font-weight: 900; color: #10b981;">R$ ${netProfit.toLocaleString('pt-BR')}</div>
                            <div style="font-size: 9px; color: #64748b;">Receita - Custos Variáveis</div>
                        </div>
                        <div>
                            <div style="font-size: 10px; color: #94a3b8; text-transform: uppercase;">Break-even Point</div>
                            <div style="font-size: 20px; font-weight: 900; color: ${revenue >= breakEven ? '#10b981' : '#f59e0b'};">
                                ${((revenue/breakEven)*100).toFixed(1)}%
                            </div>
                            <div style="font-size: 9px; color: #64748b;">Meta de Custo Fixo (R$ 1.200)</div>
                        </div>
                        <div>
                            <div style="font-size: 10px; color: #94a3b8; text-transform: uppercase;">Custo Datastone/mês</div>
                            <div style="font-size: 20px; font-weight: 900; color: #f87171;">R$ ${estimatedCost.toLocaleString('pt-BR')}</div>
                            <div style="font-size: 9px; color: #64748b;">Est. R$ 0,32 por consulta</div>
                        </div>
                        <div style="background: rgba(0,0,0,0.2); border-radius: 8px; padding: 10px; display: flex; align-items: center; justify-content: center;">
                            <div style="text-align: center;">
                                <div style="font-size: 18px; font-weight: 900; color: #4ade80;">ROI > 3x</div>
                                <div style="font-size: 9px; color: #94a3b8; text-transform: uppercase;">Status de Monetização</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 25px;">
                    <div style="background: linear-gradient(135deg, #1e3a8a, #1e293b); border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.1);">
                        <div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; font-weight: 800; letter-spacing: 1px;">Receita Bruta (Total)</div>
                        <div style="font-size: 24px; font-weight: 900; color: white; margin: 8px 0;">R$ ${revenue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
                        <div style="font-size: 10px; color: rgba(255,255,255,0.4);">Baseado em Pix aprovados</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.03); border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; font-weight: 800; letter-spacing: 1px;">Aprovações Pendentes</div>
                        <div style="font-size: 24px; font-weight: 900; color: #f59e0b; margin: 8px 0;">${pendingCount}</div>
                        <div style="font-size: 10px; color: #475569;">Aguardando ação do admin</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.03); border-radius: 12px; padding: 20px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; font-weight: 800; letter-spacing: 1px;">Taxa de Conversão</div>
                ...
                <div style="margin-bottom: 30px;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        ${pendingPlans.map(p => `
                            <div class="admin-approval-row" style="background: rgba(245, 158, 11, 0.05); border: 1px solid rgba(245, 158, 11, 0.2); border-radius: 12px; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center;">
                                <div style="display: flex; align-items: center; gap: 15px;">
                        ...
                        `).join('')}
                        ${pendingCredits.map(c => `
                            <div class="admin-approval-row" style="background: rgba(37, 99, 235, 0.05); border: 1px solid rgba(37, 99, 235, 0.2); border-radius: 12px; padding: 15px 20px; display: flex; justify-content: space-between; align-items: center;">
                                <div style="display: flex; align-items: center; gap: 15px;">
