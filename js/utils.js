...
window.formatDocument = function(doc, visible) {
    ...
    // NOVO: Se visible for EXPLICITAMENTE false, aplicamos a máscara mesmo para Master.
    // Isso permite que o Admin veja como a máscara fica (stars ***).
    const role = String(window.Monetization?.userRole || 'user').toLowerCase();
    const isMaster = role === 'admin' || role === 'master';

    // Se visible for true (vindo do isUnlockedPerson ou clique no olho), mostramos formatado.
    if (visible === true || (isMaster && visible !== false)) {
        if (clean.length === 11) {
        ...
        // Mascarado - Obscurece parcialmente (ex: 123.***.***-99)
        if (clean.length === 11) {
...
window.toggleCpfVisibility = function (btn, docValue) {
    ...
    // PADRÃO DE ÍCONES:
    // fa-eye: Dado Escondido (Clique para VER)
    // fa-eye-slash: Dado Visível (Clique para ESCONDER)
    
    const isHidden = btn.classList.contains('fa-eye');
    
    // 1. Verificar se está desbloqueado ou se é Master
...
/**
 * Extrai o DDD de uma string de telefone brasileira
 */
window.extractDDD = function(phone) {
    if (!phone) return "";
    const clean = phone.replace(/\D/g, '');
    // Handle +55 or just 11...
    const startIdx = clean.startsWith('55') ? 2 : 0;
    return clean.substring(startIdx, startIdx + 2);
};

/**
 * Normaliza e-mail (trim e lowercase)
 */
